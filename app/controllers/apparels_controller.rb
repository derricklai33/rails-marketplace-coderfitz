class ApparelsController < ApplicationController
  ...
  def index
    if params[:order] == "Price: Low to High"
      @apparel = Listing.order(price: :asc).where(category: 0, sold: 0)
      render :index
    elsif params[:order] == "Price: High to Low"
      @apparel = Listing.order(price: :desc).where(category: 0, sold: 0)
      render :index
    elsif params[:order] == "Product: A to Z"
      @apparel = Listing.order(title: :asc).where(category: 0, sold: 0)
      render :index
    elsif params[:order] == "Product: Z to A"
      @apparel = Listing.order(title: :desc).where(category: 0, sold: 0)
      render :index
    end
